...
const totalContribution = GAMES_JSON.reduce( (sum, games) => {
...
const totalRaised = GAMES_JSON.reduce( (sum, games) => {return sum + games.pledged;}, 0);
...
const totalUnfunded = GAMES_JSON.reduce( (sum, games) => {
    return sum + (games.pledged <games.goal ?1 : 0);
}, 0);

// create a string that explains the number of unfunded games using the ternary operator

const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. Currently, ${totalUnfunded} ${totalUnfunded == 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;
// create a new DOM element containing the template string and append it to the description container
const displayStringElement = document.createElement("p");
displayStringElement.innerText = displayStr;
